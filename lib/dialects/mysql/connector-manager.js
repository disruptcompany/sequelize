...
module.exports = (function() {
  ...
  ConnectorManager.prototype.query = function(sql, callee, options) {
    ...
    if (this.useQueue) {
      ...
      self.sequelize._metric("ConnectorManager.query", tk.time() - start);
  ...
  var enqueue = function(queueItem) {
    ...
    if(this.activeQueue.length < this.maxConcurrentQueries) {
      ...
      {
        self.sequelize._metric("enqueue", tk.time() - start);
  ...
  var execQueueItem = function(queueItem) {
    ...
    this.sequelize._metric("execQueueItem", tk.time() - start);
